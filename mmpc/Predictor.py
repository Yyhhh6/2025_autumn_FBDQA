...
def preprocess(x: Union[List[pd.DataFrame], pd.DataFrame], N=None, is_train=False):
    ...
    raw_cols1 = ["n_close", "sym",
        ...
        'voi', 'buy_burning', 'sell_burning', 'vol_std_ratio', 'mid_skew', # sym1
        'voi_accel', 'price_overheat', 'divergence_signal', 'kurtosis_60', # sym7
    ]
    ...
    for i, df in enumerate(x):
        ...
        # 对 sym7
        # 1. 加速特征 (针对大涨跌预测)
        extra_feats['voi_accel'] = df['voi'].diff().rolling(5, min_periods=1).mean()
        # 2. 衰竭特征 (针对趋势末端纠偏)
        # 乖离率：当前价偏离 MA20 的标准差倍数
        ma20_std = df['mid_diff1'].rolling(20, min_periods=1).std()
        extra_feats['price_overheat'] = (df['mid_price'] - df['mid_price_ma20']) / (ma20_std + 1e-6)
        # 3. 量价背离 (针对反转信号)
        # 如果价格斜率为正但 VOI 均值为负，说明是虚涨
        extra_feats['divergence_signal'] = df['mid_lr_k'] * df['voi'].rolling(20, min_periods=1).mean()
        # 4. 极端行情预警
        extra_feats['kurtosis_60'] = df['mid_diff1'].rolling(60, min_periods=1).kurt()

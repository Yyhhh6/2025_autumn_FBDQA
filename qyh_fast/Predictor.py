...
class Predictor():
    def __init__(self):
        ...
        pth_path = os.path.join(os.path.dirname(__file__), 'model_20_all_20260102_132520.json')
...
def preprocess_slice(x: list[pd.DataFrame]):
    ...
    for df in tqdm(
        ...
        for lag in lags3:
            ...
            feat_dict[f'trend_regime_lag{lag}'] = (
                (r2_30 > r2_threshold) &
                (np.abs(k_30) > k_threshold)
            ).astype(int)
            feat_dict[f'trend_strength_gated_lag{lag}'] = feat_dict[f'mid_trend_strength_lag{lag}'] * feat_dict[f'trend_regime_lag{lag}']
...
def preprocess_local(x: Union[List[pd.DataFrame], pd.DataFrame], is_train=False, is_slice=False):
    ...
    # for i, df in enumerate(x):
        ...
        r2_threshold = 0.4
        df['trend_regime'] = (
            (df['mid_lr_r2'] > r2_threshold) &
            (np.abs(df['mid_lr_k']) > k_threshold)
        ).astype(int)

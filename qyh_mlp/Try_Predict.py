...
class Predictor():
    def __init__(self):
        # 指定模型路径，不使用相对路径
        pth_path = os.path.join(os.path.dirname(__file__), 'checkpoints/model_epoch60.pth')
    ...
    def predict(self, x: List[pd.DataFrame]) -> List[List[int]]:
        # 输入x: List[pd.DataFrame]，List大小为config.json中的batch; 
            # pd.DataFrame为过去100个tick的数据; 
            # pd.DataFrame的列名为配置文件中的feature名
        # 输出List[List[int]]格式的预测结果：
            # 外层List大小为batch，即配置文件中的batch
            # 内层List大小为label个数，即配置文件中的label的个数（好像可以用不同模型来预测不同的label？？）
            # int为预测标签(0/1/2)
        # 对输入数据进行预处理
...
if __name__ == "__main__":
    csv_path = "./data/data_try/snapshot_sym0_date0_am.csv"  # ← 改成你的 CSV

    df = pd.read_csv(csv_path)

    # 假设这个 CSV 就是一个样本（100 tick）
    x = [df]

    predictor = Predictor()
    x_hat = predictor.predict(x)

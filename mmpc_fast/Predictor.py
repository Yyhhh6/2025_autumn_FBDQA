...
class Predictor():
    def __init__(self):
        ...
        pth_path = os.path.join(os.path.dirname(__file__), 'model_20_all_20260104_161412.json')
...
def compare_dfs(df1, df2, tol=1e-3):
    # --- 统一转成 DataFrame ---
    ...
    # common_cols = df1.columns.intersection(df2.columns).difference(['time_interval_lag1'])
    ...
    diff_mask = ~(df1_common.eq(df2_common) | 
              (df1_common.isna() & df2_common.isna()) | 
              ((df1_common - df2_common).abs() <= tol))
...
def preprocess_platform(x: Union[List[pd.DataFrame], pd.DataFrame], is_local=True, is_upload=True):
    ...
    if is_upload==False:    
        ...
        # # x_extract1 = pd.read_csv("x_extract1.csv", index_col=0)
        # # x_extract2 = pd.read_csv("x_extract2.csv", index_col=0)
...
def preprocess_local(x: Union[List[pd.DataFrame], pd.DataFrame], is_train=False, is_slice=False):
    ...
    raw_cols3 = [
        ...
        # "pv_corr_20", "pv_corr_60",
        "pv_z_spread_20", "pv_z_spread_50", "cum_signed_amount_10", "cum_signed_amount_30"
    ]
    ...
    # for i, df in enumerate(x):
        ...
        for w in [10, 30]:
            # 逻辑：过去 W 个 tick 内，价格上涨时的成交额之和 vs 下跌时的成交额之和
            pos_flow = (df['amount'] * (df['mid_diff1'] > 0)).rolling(w, min_periods=1).sum()
            neg_flow = (df['amount'] * (df['mid_diff1'] < 0)).rolling(w, min_periods=1).sum()
            extra_feats[f'net_amount_ratio_{w}'] = (pos_flow - neg_flow) / (pos_flow + neg_flow + 1e-10)
        # Cumulative Signed Amount (累积方向性成交额)
        extra_feats['cum_signed_amount_10'] = extra_feats['signed_amount'].rolling(window=10, min_periods=1).sum()
        extra_feats['cum_signed_amount_30'] = extra_feats['signed_amount'].rolling(window=30, min_periods=1).sum()
        for w in [20, 50]:
            # 1. 价格与成交量的滚动相关性 (Trend Confirmation)
            # 相关性趋近 -1 表示极度背离，趋近 1 表示量价同步。
            # 这是树模型最喜欢的“交互特征”，能直接区分趋势的真伪。

...
def classification_metrics(y_true, y_pred):
    metrics = {
        ...
        "f0.5_macro": f1_score(y_true, y_pred, average="macro", zero_division=0, beta=0.5),
...
def train_sgd_incremental(X_train, y_train, X_val, y_val, classes=[0,1,2], max_epochs=3, batch_size=100000):
    ...
    for epoch in range(max_epochs):
        ...
        print(f"[SGD] epoch {epoch} val f0.5_macro = {f1_score(y_val, val_pred, average='macro', zero_division=0, beta=0.5):.4f} precision_macro = {precision_score(y_val, val_pred, average='macro', zero_division=0):.4f} recall_macro = {recall_score(y_val, val_pred, average='macro', zero_division=0):.4f}")
    return sgd, scaler
...
def train_pytorch_mlp(X_train, y_train, X_val, y_val,
    ...
    for epoch in range(epochs):
        ...
        val_f0_5 = f1_score(trues, preds, average='macro', zero_division=0, beta=0.5)
